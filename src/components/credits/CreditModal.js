...
const CreditModal = ({
  ...
  const fetchAccounts = async () => {
    ...
    try {
      ...
      setAccountsList(response.data.length ? response.data : accounts);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const creditData = {
        Id: credit?.id || null, // Include ID for updates
        ...
        OdemeTakvim: paymentScheduleMap[formData.paymentSchedule] || 1,
